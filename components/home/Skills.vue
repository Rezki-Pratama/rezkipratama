<template>
    <div class="skill">
        <div class="container mx-auto px-20 title-Skill">
          <h1>Skills</h1>
          <div class="underline-skill"></div>
        </div>
        <div class="container mx-auto px-20 content-skill">
            <div class="card">
                <div class="box">
                    <div class="percent">
                        <div class="number">
                            <svg xmlns="http://www.w3.org/2000/svg" id="layer_1" xmlns:xlink="http://www.w3.org/1999/xlink" width="350" height="350" viewBox="0 0 350 350">
                            <g id="html5" transform="translate(-1102 -5391)">
                                <g id="Ellipse_52" data-name="Ellipse 52" transform="translate(1102 5391)" fill="#00b4d8" stroke="#00b4d8" stroke-width="1">
                                <circle cx="175" cy="175" r="175" stroke="none"/>
                                <circle cx="175" cy="175" r="174.5" fill="none"/>
                                </g>
                                <g id="Mask_Group_11" data-name="Mask Group 11" transform="translate(30)" clip-path="url(#clip-path)">
                                <g id="html-5" transform="translate(1160.766 5444)">
                                    <path id="Path_752" data-name="Path 752" d="M173.167,47.952,157.419,224.375,86.629,244,16.034,224.4.3,47.952H173.167Z" fill="#56e3ff"/>
                                    <path id="Path_753" data-name="Path 753" d="M86.734,229l57.2-15.858L157.394,62.379H86.734Z" fill="#b5f3ff"/>
                                    <path id="Path_754" class="sub-svg" data-name="Path 754" d="M56.12,105.661H86.734V84.021H32.469l.518,5.806,5.319,59.637H86.734V127.822H58.1ZM61,160.284H39.273l3.032,33.98,44.33,12.305.1-.027V184.026l-.095.025-24.1-6.508L61,160.284Z" fill="#ebebeb"/>
                                    <path id="Path_755" class="sub-svg" data-name="Path 755" d="M16.385,0h11V10.866H37.443V0h11V32.9h-11V21.886H27.383V32.9h-11V0ZM62.9,10.912h-9.68V0H83.589V10.912H73.9V32.9h-11V10.912h0ZM88.408,0H99.875l7.054,11.561L113.976,0h11.472V32.9H114.5V16.595l-7.566,11.7h-.19l-7.571-11.7V32.9H88.408Zm42.513,0h11V22.028h15.466V32.9H130.921V0Z" fill="#fff"/>
                                    <path id="Path_756" class="sub-svg" data-name="Path 756" d="M86.66,149.463h26.648L110.8,177.53l-24.137,6.515v22.514l44.365-12.3.325-3.656,5.086-56.974.528-5.811H86.66v21.641Zm0-43.854v.053h52.273l.434-4.864.987-10.971.517-5.806H86.66v21.589Z" fill="#fff"/>
                                </g>
                                </g>
                            </g>
                            </svg>
                        </div>
                    </div>
                    <h2 class="text">HTML</h2>
                </div>
            </div>

            <div class="card">
                <div class="box">
                    <div class="percent">
                        <div class="number">
                            <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="350" height="350" viewBox="0 0 350 350">
                            <g id="css" transform="translate(-1654 -5391)">
                                <g id="Ellipse_53" data-name="Ellipse 53" transform="translate(1654 5391)" fill="#00b4d8" stroke="#00b4d8" stroke-width="1">
                                <circle cx="175" cy="175" r="175" stroke="none"/>
                                <circle cx="175" cy="175" r="174.5" fill="none"/>
                                </g>
                                <g id="Mask_Group_12" data-name="Mask Group 12" transform="translate(-20.304 -16.184)" clip-path="url(#clip-path)">
                                <g id="css3" transform="translate(1708.055 5444.018)">
                                    <path id="Path_757" data-name="Path 757" d="M0,0H283.187V278.166H0Z" fill="none"/>
                                    <path id="Path_758" data-name="Path 758" d="M205.816,233.377,134.2,252.32l-71.62-18.942L46.05,57.5h176.3Z" transform="translate(7.396 8.052)" fill="#56e3ff"/>
                                    <path id="Path_759" data-name="Path 759" d="M122,70V235.462l.162.043,57.958-15.33L193.5,70H122Z" transform="translate(19.593 9.802)" fill="#b5f3ff"/>
                                    <path id="Path_760" class="sub-svg" data-name="Path 760" d="M88.591,37.531V27.063h15.548V15.609H76.929V47.983h27.21V37.531Zm43.518-10.468h10.87V15.609H115.767V27.063c3.641,3.576,5.539,5.288,10.8,10.46-3.078,0-10.8.011-10.8.01v10.45h27.212V37.531Zm38.568,0h10.87V15.609H154.334V27.063c3.641,3.576,5.54,5.288,10.806,10.46-3.077,0-10.806.011-10.806.01v10.45h27.214V37.531l-10.87-10.468Z" transform="translate(12.355 2.186)" fill="#fff"/>
                                    <path id="Path_761" class="sub-svg" data-name="Path 761" d="M129.037,111.333,77.777,132.3l1.7,20.762,49.56-20.815,52.719-22.14,2.186-21.239-54.905,22.462Z" transform="translate(12.491 12.444)" fill="#fff"/>
                                    <path id="Path_762" class="sub-svg" data-name="Path 762" d="M77.777,129.544l1.7,20.762,49.56-20.815V108.574Z" transform="translate(12.491 15.203)" fill="#FFFFFF"/>
                                    <path id="Path_763" class="sub-svg" data-name="Path 763" d="M176.849,88.872l-54.905,22.461V132.25l52.719-22.14Z" transform="translate(19.584 12.444)" fill="#FFFFFF"/>
                                    <path id="Path_764" class="sub-svg" data-name="Path 764" d="M77.845,126.968l1.7,20.762,76.044.238-1.7,27.682-25.023,6.92L104.811,176.6l-1.458-17.182H81l2.916,33.171,45.19,13.125,44.945-12.887,5.831-65.863Z" transform="translate(12.502 17.779)" fill="#fff"/>
                                    <path id="Path_765" class="sub-svg" data-name="Path 765" d="M129.037,126.968H77.777l1.7,20.762,49.56.155V126.968Zm0,55.536-.243.067L104.743,176.6l-1.458-17.182H80.936l2.915,33.171,45.187,13.125Z" transform="translate(12.491 17.779)" fill="#ebebeb"/>
                                    <path id="Path_766" class="sub-svg" data-name="Path 766" d="M73.8,88.872H184.581L182.4,110.11H76.472L73.8,88.872Z" transform="translate(11.852 12.444)" fill="#FFFFFF"/>
                                    <path id="Path_767" class="sub-svg" data-name="Path 767" d="M129.676,88.872H73.8l2.672,21.239h53.2V88.872Z" transform="translate(11.852 12.444)" fill="#ebebeb"/>
                                </g>
                                </g>
                            </g>
                            </svg>
                        </div>
                    </div>
                    <h2 class="text">CSS</h2>
                </div>
            </div>

            <div class="card">
                <div class="box">
                    <div class="percent">
                        <div class="number">
                            <svg xmlns="http://www.w3.org/2000/svg" id="layer_2" xmlns:xlink="http://www.w3.org/1999/xlink" width="350" height="350" viewBox="0 0 350 350">
                            <g id="sass" transform="translate(-2685 -5391)">
                                <g id="Ellipse_54" data-name="Ellipse 54" transform="translate(2685 5391)" fill="#00b4d8" stroke="#00b4d8" stroke-width="1">
                                <circle cx="175" cy="175" r="175" stroke="none"/>
                                <circle cx="175" cy="175" r="174.5" fill="none"/>
                                </g>
                                <g id="Mask_Group_13" data-name="Mask Group 13" transform="translate(479)" clip-path="url(#clip-path)">
                                <g id="sass-seeklogo.com" transform="translate(2255.554 5471.436)">
                                    <path id="Path_768" class="sub-svg" data-name="Path 768" d="M216.577,108.656a51.953,51.953,0,0,0-22.739,5.272c-2.338-4.631-4.677-8.756-5.089-11.782-.459-3.53-1.009-5.685-.459-9.9s3.026-10.224,2.98-10.682-.55-2.613-5.593-2.659-9.4.963-9.9,2.292a55.339,55.339,0,0,0-2.109,7.473c-.871,4.585-10.086,20.952-15.358,29.525-1.7-3.347-3.163-6.282-3.484-8.62-.458-3.53-1.009-5.685-.458-9.9s3.026-10.224,2.98-10.682-.551-2.613-5.593-2.659-9.4.963-9.9,2.292-1.055,4.447-2.109,7.473-13.3,30.35-16.5,37.41c-1.651,3.622-3.071,6.511-4.08,8.482h0a2.106,2.106,0,0,1-.184.367c-.871,1.7-1.374,2.613-1.374,2.613v.045c-.688,1.238-1.422,2.384-1.788,2.384-.275,0-.78-3.3.091-7.794,1.834-9.49,6.189-24.252,6.144-24.757,0-.275.825-2.842-2.842-4.173-3.577-1.329-4.86.871-5.181.871s-.55.78-.55.78,3.989-16.6-7.611-16.6c-7.243,0-17.238,7.931-22.189,15.083-3.118,1.7-9.765,5.318-16.871,9.215-2.7,1.513-5.5,3.026-8.115,4.447-.183-.183-.367-.413-.55-.6C44.1,108.885,18.155,98.294,19.255,78.168,19.668,70.832,22.19,51.577,69.09,28.2c38.6-19.026,69.32-13.754,74.638-2.063,7.61,16.688-16.459,47.68-56.345,52.173-15.221,1.7-23.2-4.172-25.215-6.373-2.109-2.292-2.43-2.43-3.209-1.971-1.284.688-.458,2.751,0,3.943,1.192,3.118,6.1,8.619,14.4,11.324,7.335,2.384,25.169,3.714,46.763-4.63,24.161-9.353,43.05-35.347,37.5-57.124-5.547-22.1-42.225-29.387-76.93-17.055-20.631,7.335-43,18.889-59.1,33.926C2.476,58.225-.55,73.767.688,80.277,5.135,103.383,37,118.421,49.743,129.561c-.642.367-1.238.688-1.742.963-6.373,3.164-30.671,15.863-36.723,29.3-6.877,15.221,1.1,26.132,6.373,27.6,16.367,4.539,33.193-3.621,42.224-17.1s7.931-30.992,3.759-39.015a.961.961,0,0,0-.183-.275c1.65-.963,3.347-1.971,5-2.934,3.255-1.926,6.464-3.713,9.215-5.18A60.413,60.413,0,0,0,74.408,139.6c-.688,8.62,2.842,19.805,7.473,24.207a8.26,8.26,0,0,0,6.052,1.971c5.41,0,7.84-4.493,10.545-9.811,3.3-6.51,6.281-14.074,6.281-14.074s-3.713,20.447,6.373,20.447c3.667,0,7.381-4.768,9.032-7.2v.045s.091-.138.275-.458c.367-.6.6-.963.6-.963v-.091c1.467-2.568,4.767-8.391,9.674-18.064,6.327-12.47,12.425-28.057,12.425-28.057a79.592,79.592,0,0,0,2.429,10.132c1.1,3.713,3.393,7.794,5.227,11.736-1.467,2.064-2.384,3.209-2.384,3.209l.046.046c-1.193,1.558-2.476,3.255-3.9,4.905-5,5.96-10.957,12.791-11.782,14.763-.963,2.338-.733,4.035,1.1,5.41,1.33,1.009,3.713,1.145,6.144,1.009a28.228,28.228,0,0,0,9.214-2.109,30.916,30.916,0,0,0,7.931-4.173,17.748,17.748,0,0,0,7.611-15.634,31.57,31.57,0,0,0-2.889-11.049c.459-.641.871-1.283,1.33-1.925a210.11,210.11,0,0,0,13.754-23.749A79.577,79.577,0,0,0,179.4,110.26c.917,3.209,2.8,6.694,4.447,10.086-7.289,5.915-11.782,12.791-13.387,17.284-2.888,8.344-.642,12.1,3.622,12.974,1.925.413,4.676-.5,6.694-1.375a30.486,30.486,0,0,0,8.482-4.355c4.905-3.622,9.627-8.666,9.352-15.5a29.3,29.3,0,0,0-2.109-9.17c6.189-2.567,14.167-3.988,24.345-2.8,21.823,2.567,26.132,16.184,25.307,21.914s-5.41,8.849-6.923,9.811-2.017,1.284-1.88,1.971c.183,1.009.916.964,2.2.78,1.788-.321,11.462-4.631,11.874-15.175.688-13.478-12.149-28.195-34.843-28.057ZM48.23,165.413c-7.244,7.885-17.33,10.865-21.685,8.344-4.676-2.706-2.842-14.351,6.052-22.694a115.522,115.522,0,0,1,17.009-12.7c1.054-.642,2.613-1.558,4.493-2.7.321-.184.5-.275.5-.275h0l1.1-.687c3.255,11.919.138,22.419-7.473,30.717Zm52.723-35.852c-2.522,6.143-7.794,21.869-11,21-2.751-.733-4.447-12.654-.55-24.436,1.971-5.915,6.143-12.975,8.573-15.725,3.943-4.4,8.3-5.869,9.352-4.08,1.33,2.338-4.814,19.392-6.372,23.243Zm43.508,20.814c-1.055.551-2.063.917-2.522.642-.321-.184.459-.917.459-.917s5.456-5.868,7.61-8.527c1.238-1.558,2.7-3.392,4.264-5.456v.6c0,7.015-6.786,11.737-9.811,13.662Zm33.559-7.656c-.78-.551-.642-2.384,1.971-8.115a30.552,30.552,0,0,1,7.473-9.627,12.977,12.977,0,0,1,.733,4.218c-.046,8.849-6.372,12.149-10.178,13.525Z" fill="#fff"/>
                                </g>
                                </g>
                            </g>
                            </svg>
                        </div>
                    </div>
                    <h2 class="text">SASS</h2>
                </div>
            </div>

            <div class="card">
                <div class="box">
                    <div class="percent">
                        <div class="number">
                            <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="350" height="350" viewBox="0 0 350 350">
                            <g id="bootstrap" transform="translate(-2150 -5391)">
                                <g id="Ellipse_55" data-name="Ellipse 55" transform="translate(2150 5391)" fill="#00b4d8" stroke="#00b4d8" stroke-width="1">
                                <circle cx="175" cy="175" r="175" stroke="none"/>
                                <circle cx="175" cy="175" r="174.5" fill="none"/>
                                </g>
                                <g id="Mask_Group_14" data-name="Mask Group 14" transform="translate(-591 21)" clip-path="url(#clip-path)">
                                <g id="bootstrap-seeklogo.com" transform="translate(2811.023 5444)">
                                    <path id="Path_769" data-name="Path 769" d="M0,176.428c0,14.425,12.167,26.116,27.176,26.116H183.587c15.01,0,27.176-11.693,27.176-26.116V26.116C210.763,11.691,198.6,0,183.587,0H27.176C12.165,0,0,11.693,0,26.116Z" fill="#56e3ff"/>
                                    <path id="Path_770" class="sub-svg" data-name="Path 770" d="M96.757,97.771V69.087h26.272a42.522,42.522,0,0,1,7.238.6,18.556,18.556,0,0,1,6.166,2.147,11.653,11.653,0,0,1,4.289,4.294,13.8,13.8,0,0,1,1.609,7.042q0,7.729-4.826,11.164t-12.332,3.435ZM68.7,48.133V170.767h61.839a67.743,67.743,0,0,0,16.711-2.061,45.565,45.565,0,0,0,14.477-6.355,31.724,31.724,0,0,0,10.1-11.078,32.676,32.676,0,0,0,3.753-16.06q0-11.508-5.808-19.666t-17.605-11.422q8.579-3.95,12.958-10.134T169.5,78.534q0-8.588-2.949-14.428a23.847,23.847,0,0,0-8.311-9.361,36.694,36.694,0,0,0-12.868-5.067,82.588,82.588,0,0,0-16.621-1.546Zm28.06,101.68V116.148h30.561q9.115,0,14.656,4.037t5.54,13.483a16.3,16.3,0,0,1-1.7,7.9,13.153,13.153,0,0,1-4.558,4.9,19.747,19.747,0,0,1-6.613,2.577,38.925,38.925,0,0,1-7.864.772Z" transform="translate(-9.358 -8.178)" fill="#fff"/>
                                </g>
                                </g>
                            </g>
                            </svg>
                        </div>
                    </div>
                    <h2 class="text">BOOTSTRAP</h2>
                </div>
            </div>

            <div class="card">
                <div class="box">
                    <div class="percent">
                        <div class="number">
                            <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="350" height="350" viewBox="0 0 350 350">
                            <g id="flutter" transform="translate(-580 -5391)">
                                <g id="Ellipse_51" data-name="Ellipse 51" transform="translate(580 5391)" fill="#00b4d8" stroke="#00b4d8" stroke-width="1">
                                <circle cx="175" cy="175" r="175" stroke="none"/>
                                <circle cx="175" cy="175" r="174.5" fill="none"/>
                                </g>
                                <g id="Mask_Group_10" data-name="Mask Group 10" transform="translate(20 53)" clip-path="url(#clip-path)">
                                <g id="flutter-2" data-name="flutter" transform="translate(604.289 5391)">
                                    <path id="Path_747" class="sub-svg" data-name="Path 747" d="M124.854,0,0,121.9l38.644,37.73L202.144,0Z" transform="translate(-0.001 -0.001)" fill="#fff"/>
                                    <path id="Path_748" class="sub-svg" data-name="Path 748" d="M158,183.691l-66.85,65.268,38.79,38.438,38.573-37.66,67.647-66.045Z" transform="translate(-34.018 -71.278)" fill="#fff"/>
                                    <path id="Path_749" data-name="Path 749" d="M153.036,329.273l29.365,28.67h76.854l-67.647-66.33Z" transform="translate(-57.113 -113.155)" fill="#56e3ff"/>
                                    <path id="Path_750" data-name="Path 750" d="M90.457,267.112l38.645-37.73,39.152,38.084-38.572,37.66Z" transform="translate(-33.759 -89.007)" fill="#b5f3ff"/>
                                    <path id="Path_751" data-name="Path 751" d="M153.036,331.293l32.119-10.406,3.191-24.069Z" transform="translate(-57.113 -115.175)" fill="url(#linear-gradient)"/>
                                </g>
                                </g>
                            </g>
                            </svg>
                        </div>
                    </div>
                    <h2 class="text">FLUTTER</h2>
                </div>
            </div>

            <div class="card">
                <div class="box">
                    <div class="percent">
                        <div class="number">
                            <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="350" height="350" viewBox="0 0 350 350">
                            <g id="tailwind" transform="translate(-3389 -5391)">
                                <g id="Ellipse_56" data-name="Ellipse 56" transform="translate(3389 5391)" fill="#00b4d8" stroke="#00b4d8" stroke-width="1">
                                <circle cx="175" cy="175" r="175" stroke="none"/>
                                <circle cx="175" cy="175" r="174.5" fill="none"/>
                                </g>
                                <g id="Mask_Group_15" data-name="Mask Group 15" transform="translate(40)" clip-path="url(#clip-path)">
                                <path id="tailwindcss-icon" class="sub-svg" d="M62.75,100.4Q75.3,50.2,125.5,50.2c50.2,0,56.475,37.65,81.575,43.925Q232.177,100.4,251,75.3q-12.548,50.2-62.75,50.2c-50.2,0-56.475-37.65-81.575-43.925Q81.573,75.3,62.75,100.4ZM0,175.7q12.548-50.2,62.75-50.2c50.2,0,56.475,37.65,81.575,43.925q25.1,6.277,43.925-18.825Q175.7,200.8,125.5,200.8c-50.2,0-56.475-37.65-81.575-43.925Q18.823,150.6,0,175.7Z" transform="translate(3399 5440.5)" fill="#fff" fill-rule="evenodd"/>
                                </g>
                            </g>
                            </svg>
                        </div>
                    </div>
                    <h2 class="text">TAILWIND CSS</h2>
                </div>
            </div>

            <div class="card">
                <div class="box">
                    <div class="percent">
                        <div class="number">
                            <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="350" height="350" viewBox="0 0 350 350">
                            <g id="vue" transform="translate(-3974 -5391)">
                                <g id="Ellipse_57" data-name="Ellipse 57" transform="translate(3974 5391)" fill="#00b4d8" stroke="#00b4d8" stroke-width="1">
                                <circle cx="175" cy="175" r="175" stroke="none"/>
                                <circle cx="175" cy="175" r="174.5" fill="none"/>
                                </g>
                                <g id="Mask_Group_16" data-name="Mask Group 16" transform="translate(1 49)" clip-path="url(#clip-path)">
                                <g id="vue-js-1" transform="translate(4024 5455.434)">
                                    <path id="Path_771" class="sub-svg" data-name="Path 771" d="M152.914,0,124.224,43.114,95.533,0H0L124.224,186.694,248.447,0Z" fill="#fff"/>
                                    <path id="Path_772" data-name="Path 772" d="M153.425,0,124.734,43.114,96.043,0H50.2l74.534,112.011L199.268,0Z" transform="translate(-0.511)" fill="#56e3ff"/>
                                </g>
                                </g>
                            </g>
                            </svg>
                        </div>
                    </div>
                    <h2 class="text">VUE</h2>
                </div>
            </div>

            <div class="card">
                <div class="box">
                    <div class="percent">
                        <div class="number">
                            <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="350" height="350" viewBox="0 0 350 350">
                            <g id="xd" transform="translate(-4210 -5409)">
                                <g id="Ellipse_58" data-name="Ellipse 58" transform="translate(4210 5409)" fill="#00b4d8" stroke="#00b4d8" stroke-width="1">
                                <circle cx="175" cy="175" r="175" stroke="none"/>
                                <circle cx="175" cy="175" r="174.5" fill="none"/>
                                </g>
                                <g id="Mask_Group_17" data-name="Mask Group 17" transform="translate(-300 36)" clip-path="url(#clip-path)">
                                <g id="adobe-xd-1" transform="translate(4576.322 5444)">
                                    <path id="Path_773" class="sub-svg" data-name="Path 773" d="M178.865,209.014H38.489C17.206,209.014,0,192.044,0,171.053V37.961C0,16.97,17.206,0,38.489,0H178.865c21.283,0,38.489,16.97,38.489,37.961V171.053C217.354,192.044,200.148,209.014,178.865,209.014Z" fill="#b5f3ff"/>
                                    <path id="Path_774" data-name="Path 774" d="M176.794,201.574H43.3c-18.11,0-32.873-14.56-32.873-32.422V42.852c0-17.862,14.763-32.422,32.873-32.422h133.4a32.646,32.646,0,0,1,32.873,32.422V169.067a32.594,32.594,0,0,1-32.786,32.508Z" transform="translate(-1.371 -1.496)" fill="#56e3ff"/>
                                    <path id="Path_775" class="sub-svg" data-name="Path 775" d="M92.084,106.314l25.361,48.054c.452.712.183,1.432-.548,1.432H101.135a1.778,1.778,0,0,1-1.9-1.158c-5.8-11.791-11.685-23.581-17.753-36.263h-.183c-5.434,11.971-11.415,24.567-17.206,36.358a1.751,1.751,0,0,1-1.635.986H47.526c-.9,0-1-.712-.539-1.252L71.8,107.841l-24-47.162a.721.721,0,0,1,.539-1.252h15.58c.9,0,1.269.18,1.626.986,5.7,11.791,11.5,23.941,16.936,35.817h.183C87.919,84.44,93.718,72.2,99.335,60.5c.452-.712.722-1.158,1.635-1.158h14.58c.722,0,1,.54.548,1.252Zm29.8,14.56c0-20.811,14.041-37.069,36.316-37.069a40.476,40.476,0,0,1,4.712.18V60.318a.9.9,0,0,1,.9-.892H178.13c.722,0,.9.266.9.712v82a62.929,62.929,0,0,0,.452,8.755c0,.54-.183.712-.722.986a54.54,54.54,0,0,1-23.187,5.179c-19.658.077-33.7-11.885-33.69-36.186Zm41.019-22.861a13.782,13.782,0,0,0-5.251-.892c-11.415,0-19.379,8.661-19.379,23.049,0,16.438,8.155,23.049,18.388,23.049a17.192,17.192,0,0,0,6.251-.986V98.014Z" transform="translate(-6.142 -8.509)" fill="#fff"/>
                                </g>
                                </g>
                            </g>
                            </svg>
                        </div>
                    </div>
                    <h2 class="text">ADOBE XD</h2>
                </div>
            </div>

            <div class="card">
                <div class="box">
                    <div class="percent">
                        <div class="number">
                            <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="350" height="350" viewBox="0 0 350 350">
                            <g id="figma" transform="translate(-557 -5994)">
                                <g id="Ellipse_59" data-name="Ellipse 59" transform="translate(557 5994)" fill="#00b4d8" stroke="#00b4d8" stroke-width="1">
                                <circle cx="175" cy="175" r="175" stroke="none"/>
                                <circle cx="175" cy="175" r="174.5" fill="none"/>
                                </g>
                                <g id="Mask_Group_18" data-name="Mask Group 18" transform="translate(-23)" clip-path="url(#clip-path)">
                                <g id="figma-1" transform="translate(674.167 6047)">
                                    <path id="path0_fill" d="M40.667,244a40.682,40.682,0,0,0,40.667-40.667V162.667H40.667a40.667,40.667,0,1,0,0,81.333Z" fill="#00d5ff"/>
                                    <path id="path1_fill" d="M0,122A40.682,40.682,0,0,1,40.667,81.333H81.333v81.333H40.667A40.682,40.682,0,0,1,0,122Z" fill="#b5f3ff"/>
                                    <path id="path1_fill_1_" d="M0,40.667A40.682,40.682,0,0,1,40.667,0H81.333V81.333H40.667A40.682,40.682,0,0,1,0,40.667Z" fill="#56e3ff"/>
                                    <path id="path2_fill" class="sub-svg" d="M81.333,0H122a40.667,40.667,0,1,1,0,81.333H81.333Z" fill="#fff"/>
                                    <path id="path3_fill" class="sub-svg" d="M162.667,122A40.667,40.667,0,1,1,122,81.333,40.682,40.682,0,0,1,162.667,122Z" fill="#fbfbfb"/>
                                </g>
                                </g>
                            </g>
                            </svg>
                        </div>
                    </div>
                    <h2 class="text">FIGMA</h2>
                </div>
            </div>

            <div class="card">
                <div class="box">
                    <div class="percent">
                        <div class="number">
                            <svg xmlns="http://www.w3.org/2000/svg" class="content-svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="350" height="350" viewBox="0 0 350 350">
                        <g id="javascript" transform="translate(-1079 -5994)">
                            <g id="Ellipse_60" data-name="Ellipse 60" transform="translate(1079 5994)" fill="#00b4d8" stroke="#00b4d8" stroke-width="1">
                            <circle cx="175" cy="175" r="175" stroke="none"/>
                            <circle cx="175" cy="175" r="174.5" fill="none"/>
                            </g>
                            <g id="Mask_Group_19" data-name="Mask Group 19" transform="translate(13 36)" clip-path="url(#clip-path)">
                            <g id="javascript-2" data-name="javascript" transform="translate(1154.493 6047)">
                                <path id="Path_776" class="sub-svg" data-name="Path 776" d="M0,0H173.811V173.155H0Z" fill="#fff"/>
                                <path id="Path_777" data-name="Path 777" d="M64.156,176.845l13.3-8.019c2.566,4.532,4.9,8.368,10.5,8.368,5.367,0,8.752-2.091,8.752-10.227V111.639h16.334V167.2c0,16.854-9.917,24.525-24.385,24.525-13.066,0-20.651-6.742-24.5-14.878m57.758-1.742,13.3-7.671c3.5,5.7,8.052,9.88,16.1,9.88,6.768,0,11.084-3.371,11.084-8.021,0-5.579-4.434-7.555-11.9-10.809l-4.083-1.745c-11.785-5-19.6-11.273-19.6-24.524,0-12.2,9.334-21.5,23.918-21.5,10.384,0,17.851,3.6,23.217,13.018l-12.718,8.137c-2.8-5-5.833-6.974-10.5-6.974-4.784,0-7.817,3.022-7.817,6.974,0,4.881,3.034,6.859,10.034,9.881l4.083,1.743c13.885,5.929,21.7,11.972,21.7,25.57,0,14.646-11.55,22.666-27.068,22.666-15.167,0-24.968-7.206-29.751-16.622" transform="translate(-18.455 -32.144)" fill="#56e3ff"/>
                            </g>
                            </g>
                        </g>
                        </svg>
                        </div>
                    </div>
                    <h2 class="text">JAVASCRIPT</h2>
                </div>
            </div>

            <div class="card">
                <div class="box">
                    <div class="percent">
                        <div class="number">
                           <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="350" height="350" viewBox="0 0 350 350">
                            <g id="laravel" transform="translate(-1650 -5994)">
                                <g id="Ellipse_63" data-name="Ellipse 63" transform="translate(1650 5994)" fill="#00b4d8" stroke="#00b4d8" stroke-width="1">
                                <circle cx="175" cy="175" r="175" stroke="none"/>
                                <circle cx="175" cy="175" r="174.5" fill="none"/>
                                </g>
                                <g id="Mask_Group_22" data-name="Mask Group 22" transform="translate(-33 -21)" clip-path="url(#clip-path)">
                                <g id="laravel-2" data-name="laravel" transform="translate(1762.942 6083)">
                                    <path id="Path_793" class="sub-svg" data-name="Path 793" d="M212.969,48.308a3.263,3.263,0,0,1,.12.873V95.017a3.327,3.327,0,0,1-1.726,2.9l-39.52,22.15v43.9a3.337,3.337,0,0,1-1.716,2.9L87.632,213.1a3.832,3.832,0,0,1-.6.242c-.077.025-.15.071-.231.092a3.555,3.555,0,0,1-1.76,0c-.095-.025-.181-.075-.271-.109a3.7,3.7,0,0,1-.566-.225L1.725,166.867A3.33,3.33,0,0,1,0,163.968V26.46a3.332,3.332,0,0,1,.12-.877c.026-.1.086-.184.12-.28a3.247,3.247,0,0,1,.219-.518,3.151,3.151,0,0,1,.236-.3A3.417,3.417,0,0,1,1,24.1a3.341,3.341,0,0,1,.339-.251,2.788,2.788,0,0,1,.378-.288h0L42.962.447a3.523,3.523,0,0,1,3.433,0L87.636,23.561h.008a4.132,4.132,0,0,1,.378.284,3.975,3.975,0,0,1,.335.251,3.766,3.766,0,0,1,.309.393,3.04,3.04,0,0,1,.232.3,3.534,3.534,0,0,1,.223.518c.034.1.094.184.12.284a3.3,3.3,0,0,1,.121.873v85.886l34.366-19.263V49.177a3.287,3.287,0,0,1,.121-.869c.03-.1.086-.188.12-.284a3.81,3.81,0,0,1,.223-.518c.064-.109.159-.2.232-.3a3.325,3.325,0,0,1,.309-.389,3.231,3.231,0,0,1,.335-.251,3.48,3.48,0,0,1,.378-.288h0l41.245-23.114a3.519,3.519,0,0,1,3.434,0l41.241,23.114a3.839,3.839,0,0,1,.386.284c.107.084.231.159.33.251a4,4,0,0,1,.309.393,2.5,2.5,0,0,1,.231.3,3.2,3.2,0,0,1,.223.518A2.293,2.293,0,0,1,212.969,48.308Zm-6.755,44.775V54.968l-14.433,8.088L171.844,74.23v38.116l34.375-19.263Zm-41.241,68.951V123.892l-19.613,10.9-56,31.114v38.5ZM6.875,32.246V162.034l75.607,42.373V165.914l-39.5-21.761-.013-.008-.017-.008a629.287,629.287,0,0,0-.7-.519l-.009-.012a3.233,3.233,0,0,1-.283-.351,3.6,3.6,0,0,1-.258-.326l0-.013a2.616,2.616,0,0,1-.18-.417,2.455,2.455,0,0,1-.163-.376v0a3.143,3.143,0,0,1-.069-.489,2.811,2.811,0,0,1-.051-.376V51.509L21.307,40.329,6.875,32.25ZM44.683,7.206,10.321,26.46,44.674,45.714,79.032,26.456,44.674,7.206Zm17.87,120.163L82.487,116.2V32.246L68.054,40.334,48.116,51.509V135.46ZM168.41,29.927,134.053,49.182,168.41,68.436l34.353-19.258Zm-3.437,44.3L145.035,63.055,130.6,54.968V93.083l19.934,11.171,14.437,8.091ZM85.916,160.125l50.395-28.006,25.191-14L127.169,98.881,87.64,121.035l-36.027,20.19Z" transform="translate(0 0)" fill="#fff"/>
                                </g>
                                </g>
                            </g>
                            </svg>
                        </div>
                    </div>
                    <h2 class="text">LARAVEL</h2>
                </div>
            </div>


        </div>
    </div>
</template>

<script>
export default {
    name: 'Skill',

}
</script>

<style lang="scss" scoped>

    @import 'assets/frontend/styles/utilities/_utilities';

    .skill {
        margin-top: 100px;
        .title-Skill {
            width: max-content;
            height: max-content;
            h1 {
                text-align: center;
                font-size: 40px;
                font-weight: bold;
                color: $colorBlack;
            }
            .underline-skill {
                background-color: $colorSecondary;
                width: 100%;
                height: 5px;
                border-radius: 20px;
            }
        }

        .content-skill {
            margin-top: 50px;
            margin-bottom: 50px;
            display: flex;
            justify-content: center;
            flex-wrap: wrap;

            .card {
                width: 200px;
                height: 250px;
                display: flex;
                margin: 0px 10px;
                justify-content: center;
                align-items: center;
                border-radius: 8px;
                text-align: center;
                overflow: hidden;
                transition: 0.5s;
                // background: linear-gradient(0deg,#1b1b1b, #222, #1b1b1b);

                .box {
                    .percent {
                        position: relative;
                        width: 150px;
                        height: 150px;
                        border-radius: 50%;

                        .number {
                            position: absolute;
                            top: 0;
                            left: 0;
                            width: 100%;
                            height: 100%;
                            display: flex;
                            justify-content: center;
                            align-items: center;
                            border-radius: 50%;
                            transition: all 0.5s ease-in-out;
                            

                            svg {
                                width: 100%;
                                height: 100%;
                                object-fit:contain;
                                padding: 20px;
                                transition: transform 0.2s ease-in-out;

                                path {
                                    transition: fill 0.5s;
                                }
                            }
                        }
                    }

                    .text {
                        position: relative;
                        margin-top: 20px;
                        color: $colorBlack;
                        font-weight: 700;
                        font-size: 18px;
                        letter-spacing: 1;
                        text-transform: uppercase;
                        transition: 0.5s;
                    }
                }

                &:hover {
                    background-color: $colorStabilo;
                    transform: translateY(-10px);
                    box-shadow: 0 15px 35px rgba(0,0,0,.5);
                }

                &:hover .text {
                    color: $colorSecondary;
                    font-size: 25px;
                }

                &:hover .number {
                    transform-box: fill-box;
                    transform-origin: 50% 50%;
                    transform: rotate3d(0, 2, 1, 380deg);
                }

                &:hover .number svg .sub-svg {
                    fill: $colorStabilo;
                }
            }

            .card:nth-child(1) svg circle:nth-child(2) {
                stroke-dashoffset: (440 - (440 * 80) / 100);
            }

            .card:nth-child(2) svg circle:nth-child(2) {
                stroke-dashoffset: (440 - (440 * 80) / 100);
            }

            .card:nth-child(3) svg circle:nth-child(2) {
                stroke-dashoffset: (440 - (440 * 80) / 100);
            }

            .card:nth-child(4) svg circle:nth-child(2) {
                stroke-dashoffset: (440 - (440 * 70) / 100);
            }

            .card:nth-child(5) svg circle:nth-child(2) {
                stroke-dashoffset: (440 - (440 * 80) / 100);
            }

            .card:nth-child(6) svg circle:nth-child(2) {
                stroke-dashoffset: (440 - (440 * 80) / 100);
            }

            .card:nth-child(7) svg circle:nth-child(2) {
                stroke-dashoffset: (440 - (440 * 60) / 100);
            }

            .card:nth-child(8) svg circle:nth-child(2) {
                stroke-dashoffset: (440 - (440 * 70) / 100);
            }

            .card:nth-child(9) svg circle:nth-child(2) {
                stroke-dashoffset: (440 - (440 * 80) / 100);
            }

            // .card:before {
            //     content: '';
            //     position: absolute;
            //     top: 0;
            //     left: -50%;
            //     width: 100%;
            //     height: 100%;
            //     pointer-events: none;
            //     background: rgba(255, 255, 255, .03);
            //     z-index: 1;
            // }
        }
    }
</style>